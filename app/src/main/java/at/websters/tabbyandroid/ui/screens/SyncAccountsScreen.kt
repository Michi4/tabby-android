...
fun SyncAccountsScreen(
    ...
    editing?.let { acc ->
        ...
        var token by remember(acc.id) { mutableStateOf(vm.getToken(acc.id)) }
        ...
        AlertDialog(
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    OutlinedTextField(value = token, onValueChange = { token = it }, label = { Text("Secret sync token") }, singleLine = true, visualTransformation = PasswordVisualTransformation())
                    Button(onClick = {
                        testError = null
                        vm.testAndList(name, host, token) { list, error ->
                            configs = list
                            testError = error
